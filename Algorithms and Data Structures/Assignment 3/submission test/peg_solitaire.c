...
int main(int argc, char *argv[]) {
	...
	if( ai_run ){ 
        ...
        fprintf(output, "\tExpanded nodes: %'d\n\tGenerated nodes: %'d\n", expanded_nodes, generated_nodes);
        fprintf(output, "\tSolution Length: %d\n", solution_size);
        fprintf(output, "\tNumber of Pegs Left: %d\n", num_pegs( &(solution[solution_size]) ) );
        fprintf(output, "\tExpanded/seconds: %d\n", (int)(expanded_nodes/cpu_time_used) );
        fprintf(output, "\tTime (seconds): %f\n", cpu_time_used );


        setBufferedInput(true);
		printf("\033[?25h\033[0m");

        // Freeing the Output File
        fclose(output);
