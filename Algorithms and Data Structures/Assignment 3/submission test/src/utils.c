	...
	for (y=0;y<SIZE;y++) {
		...
		for (x=0;x<SIZE;x++) {
			if (board->cursor.x == x && board->cursor.y == y) {
				if (board->selected) {
					printf("\b|\033[7m%c\033[27m|",board->field[x][y]); //NOTE this prints new locations (?)
				} else {
					printf("\033[7m%c\033[27m ",board->field[x][y]); //NOTE cursor
				}
			} else {
				printf("%c ",board->field[x][y]); //NOTE the places of the other pieces ~IMPORTANT~
			}
		}
		for (x=0;x<14-SIZE;x++) {
			printf(" ");
